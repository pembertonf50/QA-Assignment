...
def home(request):
  ...
  if request.method == 'POST' and user.is_authenticated:
    ...
    if country_iso_codes.get(testAccountLocation) and testAccountLanguage in languages:
      ...
      verboseLog(message='failed to create test account.',
                 ipAddress=request.META.get('HTTP_X_FORWARDED_FOR') or request.META.get('REMOTE_ADDR'),
                 userEmail=user.email,
                 testAccountLocation=testAccountLocation, testAccountLanguage=testAccountLanguage,
                 testAccountSubscriptions=testAccountSubscriptions, testAccountCardSaved=testAccountCardSaved,
                 testAccountAddressSaved=testAccountAddressSaved)
...
def logIn(request):
  if request.method == 'POST':
    ...
    if user is not None:
      ...
      verboseLog(message='failed to switch account.' if request.user.is_authenticated else 'failed to login.',
                 ipAddress=request.META.get('HTTP_X_FORWARDED_FOR') or request.META.get('REMOTE_ADDR'),
                 userEmail=request.user.email if request.user.is_authenticated else '(user not signed in)',
...
def signUp(request):
  if request.method == 'POST':
    ...
    if User.objects.filter(email=email).exists():
        ...
        verboseLog(message='email already registered.',
                   ipAddress=request.META.get('HTTP_X_FORWARDED_FOR') or request.META.get('REMOTE_ADDR'),
                   userEmail=request.user.email if request.user.is_authenticated else '(user not signed in)',
                   emailInput=email, passwordInput=password)
    try:
      ...
      verboseLog(message='invalid email used.',
                 ipAddress=request.META.get('HTTP_X_FORWARDED_FOR') or request.META.get('REMOTE_ADDR'),
                 userEmail=request.user.email if request.user.is_authenticated else '(user not signed in)',
                 emailInput=email, passwordInput=password)
    try:
      ...
      messages.error(request, messageString)
      verboseLog(message='insufficiently strong password.',
                 ipAddress=request.META.get('HTTP_X_FORWARDED_FOR') or request.META.get('REMOTE_ADDR'),
                 userEmail=request.user.email if request.user.is_authenticated else '(user not signed in)',
                 emailInput=email, passwordInput=password)
